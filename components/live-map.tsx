...
import { PlayerDetailPanel, type SelectedMapEntity } from '@/components/player-detail-panel'
import type { Guild, GuildMember } from '@/lib/guilds'
...
// Navigation, the action cluster, and the console all live in the shared
// DashboardHeader now (#5), so LiveMap takes no props -- it renders only the
// map, its compact controls strip, and the canvas.
export function LiveMap() {
  ...
  // PATCH (item 4): last-known positions of OFFLINE players from the guild
  // export (lib/guilds.ts). Off by default -- it's historical, not live.
  const [showOffline, setShowOfflineRaw] = useState(() => readLayer('offline', false))
  ...
  const setShowOffline = persistLayer('offline', setShowOfflineRaw)
  // Clicking a marker opens the per-player detail panel.
  const [selectedEntity, setSelectedEntity] = useState<SelectedMapEntity | null>(null)
  // Guild export, fetched only while the offline layer is on.
  ...
  // Fetch the guild export only while the offline layer is on. Refreshed on the
  // same cadence as the map; a failure just leaves the layer empty.
  useEffect(() => {
    ...
    const fetchGuilds = async () => {
      try {
        ...
        if (!cancelled) setGuilds((data.guilds ?? []) as Guild[])
      } catch {
        // leave whatever we had
      }
    }
    fetchGuilds()
    const interval = setInterval(fetchGuilds, REFRESH_INTERVAL_MS * 4)
    return () => {
      cancelled = true
  ...
  // Offline guild members (not currently connected), positioned with the SAME
  // world->screen transform as online players -- verified their WorldPosition
  // shares the online coordinate space. Matched to online players by name,
  // since PalDefender ids and the snapshot userId differ in format.
  const offlineMarkers = useMemo(() => {
    if (!showOffline || guilds.length === 0) return [] as Array<{
      key: string
      member: GuildMember
      guildName: string
      x: number
    ...
    const markers: Array<{ key: string; member: GuildMember; guildName: string; x: number; y: number }> = []
    for (const guild of guilds) {
      for (const member of guild.members) {
        if (!member.worldPosition) continue
        if (onlineNames.has(member.nickName.toLowerCase())) continue // shown as a live marker
        const { x, y } = toScreenPixels(
          [member.worldPosition.x, member.worldPosition.y],
          MAP_BASIS,
          MAP_BASIS,
          activeBounds,
        )
        markers.push({ key: member.id, member, guildName: guild.name, x, y })
      }
    }
    return markers
  }, [showOffline, guilds, players, activeBounds])
  ...
  return (
    <div className="flex h-full w-full flex-col bg-background text-foreground">
      <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 border-b border-border/60 bg-card/70 px-4 py-2 backdrop-blur">
        <div className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
          <span className="relative inline-flex h-2 w-2">
            ...
            <span className="status-dot relative inline-flex h-2 w-2 rounded-full bg-red-500" />
          </span>
          <span>Live Map</span>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          {/* Tab navigation lives in the shared DashboardHeader now (#5); this
              strip only carries map-specific controls. */}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                title={showPlayers ? undefined : 'Player markers are hidden — toggle "Players" back on'}
                className="relative h-10 gap-2 border-border/60 bg-muted/20 font-mono text-[11px] uppercase tracking-[0.2em]"
              >
                <LayersIcon className="h-3.5 w-3.5" />
                Layers
                {/* Layer visibility persists across reloads; without a cue, a
                    hidden Players layer looks like "my connected player vanished"
                    (verified confusion). Flag the one normally-on layer when off. */}
                {!showPlayers && (
                  <span
                    aria-label="Player markers hidden"
        ...
        >
                  ...
                  {group.players.map(({ player, x, y }, index) => {
                    ...
                    return (
                      <div
                        ...
                        className={`pointer-events-auto absolute cursor-pointer ${isHovered ? 'z-40' : 'z-30'}`}
                        ...
                        onClick={(event) => {
                          ...
                          setSelectedEntity({ kind: 'online', player })
                        }}
                      >
                        <div
          ...
          {showOffline &&
            offlineMarkers.map((marker) => (
              <div
                key={`offline-${marker.key}`}
                className="pointer-events-auto absolute z-20 cursor-pointer"
                style={{ left: `${ov.tx + marker.x * ov.scale}px`, top: `${ov.ty + marker.y * ov.scale}px` }}
                onClick={(event) => {
                  event.stopPropagation()
                  setSelectedEntity({ kind: 'offline', member: marker.member, guildName: marker.guildName })
                }}
                title={`${marker.member.nickName} (offline)`}
              >
                {/* Dimmed, hollow marker so last-known positions read as historical, not live. */}
                <div
                  className="absolute left-0 top-0 h-[14px] w-[14px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-muted-foreground/60 bg-muted-foreground/25"
                  style={{ transform: 'translate(-50%, -50%)' }}
                />
              </div>
            ))}
      ...
      <PlayerDetailPanel selected={selectedEntity} onClose={() => setSelectedEntity(null)} />
    </div>
  )
}
