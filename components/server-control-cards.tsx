...
export function ServerManagementCard() {
  ...
  // PATCH (not upstream): "Check for Updates" -- a genuinely read-only check
  // (installed vs. latest public buildid), safe to run anytime, even with
  // players online. Applying an update is just the existing Restart button
  // (ALWAYS_UPDATE_ON_START=true already makes a restart run SteamCMD's own
  // update-check-and-apply as part of normal boot) -- no separate action here.
  const [updateInfo, setUpdateInfo] = useState<{ installedBuildId: string; latestBuildId: string; updateAvailable: boolean } | null>(null)
  ...
  // that connection explicit and one click, instead of an implicit side
  // effect of a differently-labeled button.
  const applyUpdateNow = async () => {
    clearScheduleTimers()
    try {
      const ok = await triggerServerRestart(60, `Updating to build ${updateInfo?.latestBuildId ?? 'latest'}`)
      if (!ok) {
        toast.error('Failed to schedule update -- is the server host integration set up?')
        return
      }
      setActiveSchedule({ label: 'Applying update', endsAt: Date.now() + 60_000 })
      setRemaining(60_000)
      toast.success('Update scheduled -- restarting in 60 seconds to apply it')
    } catch {
  ...
  const triggerServerStop = async (waittimeSec: number, message: string): Promise<boolean> => {
    if (!config) return false
    const headers = new Headers(buildPalworldProxyHeaders(config))
    headers.set('Content-Type', 'application/json')
  ...
  // PATCH (not upstream): Palworld's REST /save endpoint has a genuine quirk
  // in this build -- a bodyless POST gets a clean 411 (missing
  // Content-Length), but adding a body (even '{}') breaks the connection
  // outright instead of fixing it. Rather than chase an upstream REST bug we
  // can't fix, this uses RCON's own Save command instead -- the exact same
  // operation, over the connection we already know is reliable (/api/rcon).
  const saveWorldAndAnnounce = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Save command failed')
  ...
  const startServer = async () => {
    ...
    try {
      const headers = new Headers(buildPalworldProxyHeaders(config))
      headers.set('Content-Type', 'application/json')
      const res = await fetch('/api/server-start', { method: 'POST', headers, cache: 'no-store' })
      if (!res.ok) {
        toast.error('Failed to start server — is the server host integration set up?')
      } else {
        toast.success('Start queued — server should be reachable again shortly')
      }
    } catch {
  ...
  const shutdownServer = async () => {
    ...
    try {
      ...
      // Queue a genuine host-level stop. The systemd handler broadcasts the
      // warning itself, waits (checking for cancellation), then runs
      // `docker compose stop` -- so the server actually stays off, unlike
      // calling the game's own REST /shutdown, which Docker's restart
      // policy would just bring straight back up.
      const ok = await triggerServerStop(10, 'Server will shutdown in 10 seconds!')
      if (!ok) {
        toast.error('Failed to queue shutdown — is the server host integration set up?')
        setIsProcessing(false)
        setConfirmAction(null)
        return
      }
      toast.success('Shutdown queued — server will stop in 10 seconds')
    } catch {
      toast.error('Failed to queue shutdown')
    }
    setIsProcessing(false)
    setConfirmAction(null)
  }
  ...
  const stopServer = async () => {
    ...
    try {
      ...
      // Queue an immediate host-level stop (no warning delay), same
      // genuine-stop mechanism as shutdownServer above.
      const ok = await triggerServerStop(0, 'Server force stopping now!')
      if (!ok) {
        toast.error('Failed to stop server — is the server host integration set up?')
        setIsProcessing(false)
        setConfirmAction(null)
        return
      }
      toast.success('Server stopping now')
    } catch {
    ...
    return (
      ...
      >
          <div className="flex flex-col gap-1.5">
            <p className="text-[11px] font-semibold text-muted-foreground">Server Version</p>
            <div className="flex items-center justify-between gap-2 rounded-lg border border-border/60 bg-muted/10 px-3 py-2 text-xs">
            <div className="flex min-w-0 items-center gap-2">
              <DownloadCloudIcon className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
              {updateInfo ? (
                updateInfo.updateAvailable ? (
                  <span className="truncate text-warning">
                    Update available -- build {updateInfo.installedBuildId} to {updateInfo.latestBuildId}
                  </span>
                ) : (
                  <span className="truncate text-muted-foreground">
                    Up to date -- build {updateInfo.installedBuildId}
                  </span>
                )
              ) : (
                <span className="truncate text-muted-foreground">{checkingUpdate ? 'Checking for updates...' : 'Update status unknown'}</span>
              )}
            </div>
            <div className="flex shrink-0 items-center gap-1.5">
              {updateInfo?.updateAvailable && (
                <Button
                  size="sm"
                  variant="outline"
                  className="h-6 !border-warning/60 !bg-warning/12 px-2 text-xs !text-warning-foreground hover:!bg-warning/22"
                  onClick={applyUpdateNow}
                  disabled={isProcessing || !!activeSchedule}
                >
                  Apply Update
                </Button>
              )}
              ...
              >
                {checkingUpdate ? <Spinner className="h-3 w-3" /> : 'Check Now'}
              </Button>
            </div>
          </div>
          </div>
          ...
          {connectionStatus === 'disconnected' ? (
            ...
            >
              {isProcessing ? <Spinner className="w-4 h-4 mr-2" /> : <PowerIcon className="w-4 h-4 mr-2" />}
              Start Server
            </Button>
          ) : (
            <>
...
export function BanManagementCard() {
  const { apiCall, isLoading, bannedPlayers, removeBannedPlayer, config } = useServer()
  ...
  // PATCH (not upstream): PalDefender's own, separate IP-ban system -- no
  // game-native equivalent. Entirely optional: if PalDefender isn't
  // installed/configured, fetches fail silently and this whole section just
  // stays hidden rather than showing an error.
  const [ipBans, setIpBans] = useState<{ ip: string; reason: string }[]>([])
  const [ipInput, setIpInput] = useState('')
  const [ipLoading, setIpLoading] = useState<Record<string, boolean>>({})
  const [ipAvailable, setIpAvailable] = useState(false)

  const paldefenderPost = async (path: string) => {
    if (!config) return null
    const headers = new Headers(buildPalworldProxyHeaders(config))
    const res = await fetch(`/api/paldefender/${path}`, { method: 'POST', headers, cache: 'no-store' })
  ...
  const fetchIpBans = async () => {
    ...
    try {
      const headers = new Headers(buildPalworldProxyHeaders(config))
      const res = await fetch('/api/paldefender/banlist', { headers, cache: 'no-store' })
      if (!res.ok) {
        setIpAvailable(false)
        return
      }
      const data = await res.json()
      const entries = data?.Banlist?.IPEntries ?? []
      setIpBans(
        entries
          .filter((e: { Active?: boolean }) => e.Active)
          .map((e: { IP: string; BannedBy?: { Reason?: string } }) => ({
            ip: e.IP,
            reason: e.BannedBy?.Reason ?? '',
          }))
      )
      setIpAvailable(true)
    } catch {
  ...
  const handleBanIp = async () => {
    ...
    if (!ip) return
    setIpLoading((prev) => ({ ...prev, [ip]: true }))
    try {
      const result = await paldefenderPost(`banip/${encodeURIComponent(ip)}`)
      if (result?.Success) {
        toast.success(`Banned IP ${ip}`)
        setIpInput('')
        await fetchIpBans()
      } else {
        toast.error(`Failed to ban IP ${ip}`)
      }
    } catch {
      toast.error(`Failed to ban IP ${ip}`)
    }
    setIpLoading((prev) => ({ ...prev, [ip]: false }))
  }

  const handleUnbanIp = async (ip: string) => {
    setIpLoading((prev) => ({ ...prev, [ip]: true }))
    try {
      const result = await paldefenderPost(`unbanip/${encodeURIComponent(ip)}`)
      if (result?.Success) {
  ...
  return (
    <PanelSection title="Ban Management" subtitle="Sanctions Ledger" status={bannedPlayers.length > 0 ? 'pending' : 'active'} className="min-h-[34rem]">
        ...
        {/* PATCH (not upstream): PalDefender IP bans -- only shown when
            PalDefender's REST API is actually reachable and configured. */}
        {ipAvailable && (
          <div className="space-y-2 mt-4 pt-4 border-t border-border/60">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
              IP Bans — PalDefender ({ipBans.length})
            </p>
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                className="shrink-0 h-8 px-2 text-xs border-destructive/50 text-destructive hover:bg-destructive/10"
                disabled={!ipInput.trim() || ipLoading[ipInput.trim()]}
                onClick={handleBanIp}
              >
                {ipLoading[ipInput.trim()] ? <Spinner className="w-3 h-3" /> : 'Ban IP'}
              </Button>
            </div>
            {ipBans.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-2">No IP bans</p>
            ) : (
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {ipBans.map((banned) => (
                  <div key={banned.ip} className="flex items-center justify-between gap-2 p-2 rounded-lg bg-secondary/50 border border-border">
                    <div className="min-w-0">
                      <p className="text-sm font-mono font-medium text-foreground truncate">{banned.ip}</p>
                      {banned.reason && <p className="text-xs text-muted-foreground truncate">{banned.reason}</p>}
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                      className="shrink-0 h-7 px-2 text-xs border-border"
                      disabled={ipLoading[banned.ip]}
                      onClick={() => handleUnbanIp(banned.ip)}
                    >
                      {ipLoading[banned.ip] ? <Spinner className="w-3 h-3" /> : 'Unban'}
                    </Button>
                  </div>
                ))}
              </div>
