...
// Multi-instance (#7): which server the operator has drilled into. null = the
// fleet landing (server list). Persisted so a reload returns to the same server.
const ACTIVE_INSTANCE_STORAGE_KEY = 'activeInstance'
...
interface ServerContextType {
  ...
  // PATCH (item B): roster quick-actions open the RCON console with a command
  // preselected and a player prefilled. The roster sets this; the dashboard
  // navigates to the RCON tab; the console consumes it and clears it. No new
  // execution path -- the send still goes through the console UI and its confirms.
  ...
  enterInstance: (id: string, ports?: { gamePort?: string; restApiPort?: string }) => void
  exitToFleet: () => void
}
...
export function ServerProvider({ children }: { children: ReactNode }) {
  ...
  // Drill into a server: carry its id on every request (header) and reset the
  // live state so the entered server's snapshot repopulates cleanly.
  const enterInstance = useCallback((id: string, ports?: { gamePort?: string; restApiPort?: string }) => {
    setActiveInstanceIdState(id)
    localStorage.setItem(ACTIVE_INSTANCE_STORAGE_KEY, id)
    setConfigState((c) => {
      if (!c) return c
      // Carry the instance's own ports so the header address + copy reflect the
      // selected server (the public IP/host stays the same box). Persist so a
      // reload restores the right ports for the drilled-into server.
      const next = { ...c, instanceId: id, gamePort: ports?.gamePort ?? c.gamePort, restApiPort: ports?.restApiPort ?? c.restApiPort }
      try {
        if (localStorage.getItem(ACTIVE_SESSION_STORAGE_KEY) === '1') writeStorageValue(STORAGE_KEYS.config, next)
      } catch {
        /* non-remembered session — in-memory only */
      }
      return next
    })
    setConnectionStatus('checking')
  ...
  // Back to the fleet landing: drop the header + clear the per-server state.
  const exitToFleet = useCallback(() => {
    setActiveInstanceIdState(null)
    localStorage.removeItem(ACTIVE_INSTANCE_STORAGE_KEY)
    setConfigState((c) => (c ? { ...c, instanceId: undefined } : c))
    setPlayersState([])
    setServerInfoState(null)
    setServerMetricsState(null)
  ...
  const fetchSnapshot = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      // ANY snapshot failure means the server is unreachable right now -- reflect
      // it. The old code only flipped to 'disconnected' for a hardcoded list of
      // error-message substrings, so a stopped server (whose 502 message didn't
      // match) left a stale "Connected" badge AND a frozen roster (verified in
      // browser QA after a Stop). This is self-healing: the next successful poll
      // sets 'connected' again and repopulates players. The fetch is to our own
      // /api/server-snapshot (the dashboard is always up), so a throw here means
      // the game REST genuinely failed, not a flaky client hop.
      setConnectionStatus('disconnected')
      setLastConnectionError(errorMessage)
      setPlayers([])
