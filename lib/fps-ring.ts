...
// Multi-instance (#7): the sampler writes each non-default instance's ring as a
// sibling `fps-history.<id>.json` in the same dir; `default` keeps HISTORY_FILE.
function historyFileFor(instanceId?: string | null): string {
  if (!instanceId || instanceId === 'default') return HISTORY_FILE
  const slash = HISTORY_FILE.lastIndexOf('/')
  const dir = slash >= 0 ? HISTORY_FILE.slice(0, slash) : '.'
  return `${dir}/fps-history.${instanceId}.json`
}
...
export async function readFpsRing(instanceId?: string | null): Promise<FpsRingPayload> {
  try {
    const raw = await readFile(/* turbopackIgnore: true */ historyFileFor(instanceId), 'utf8')
