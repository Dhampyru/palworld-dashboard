...
import { runWithInstance, currentRestConfig } from '@/lib/instances'
...
export async function GET(request: NextRequest) {
  return runWithInstance(request.headers.get(PALWORLD_PROXY_HEADERS.instance), () => getChat(request))
}
async function getChat(request: NextRequest) {
...
export async function POST(request: NextRequest) {
  return runWithInstance(request.headers.get(PALWORLD_PROXY_HEADERS.instance), () => postChat(request))
}
async function postChat(request: NextRequest) {
  ...
  const { restUrl, adminPassword: gameAdminPassword } = currentRestConfig()
  const pinned = new URL(restUrl)
